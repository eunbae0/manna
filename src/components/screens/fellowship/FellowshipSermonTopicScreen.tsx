...
export default function FellowshipSermonTopicScreen() {
	...
	const [sermonTopic, setSermonTopic] = useState<string[]>(
		content.sermonTopic.length === 0 ? [''] : content.sermonTopic,
	);
		...
		<SafeAreaView className="h-full">
			<VStack className="flex-1">
							...
							{sermonTopic.map((item, index) => (
								<HStack key={index} space="lg" className="w-full items-center">
									...
									<Pressable
										onPress={() => {
											const updatedSermonTopic = [...sermonTopic];
											updatedSermonTopic.splice(index, 1);
											setSermonTopic(updatedSermonTopic);
										}}
									>
										<Icon as={X} size="lg" />
				...
				<Button
					...
					onPress={() => {
						updateFellowshipContent({ sermonTopic });
						setStep('CONTENT');
					}}
				>
