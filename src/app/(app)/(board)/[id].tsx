...
import { BottomSheetListHeader, BottomSheetListItem, BottomSheetListLayout } from '@/components/common/bottom-sheet';
...
export default function BoardPostDetailScreen() {
  ...
  }, [currentGroup?.groupId, postId, refetchPost, refetchComments, refetchReactions, showError]);
  ...
  const handleTogglePin = async () => {
    ...
    // 고정 해제의 경우 바로 처리
    if (post.isPinned) {
      await updatePostPinStatus(post.id, currentGroup.groupId, false, handlePostSettingClose);
      return;
    }
    ...
    if (!existingPinnedPost) {
      ...
      await updatePostPinStatus(post.id, currentGroup.groupId, true, handlePostSettingClose);
      return;
    }
  ...
  const { BottomSheetContainer: LikeMemberListBottomSheetContainer, handleOpen: handleLikeMemberListOpen, handleClose: handleLikeMemberListClose } = useBottomSheet();
  const { BottomSheetContainer: PostSettingBottomSheetContainer, handleOpen: handlePostSettingOpen, handleClose: handlePostSettingClose } = useBottomSheet();
      ...
      <SafeAreaView className="flex-1">
        ...
        <ScrollView
          ...
          <VStack className="flex-1 p-4">
            ...
            {/* 게시글 통계 */}
            {reactions?.like && reactions?.like.length > 0 && <AnimatedPressable onPress={handleLikeMemberListOpen}>
        ...
        <LikeMemberListBottomSheetContainer>
          <BottomSheetListLayout>
            <BottomSheetListHeader label="공감한 멤버" onPress={handleLikeMemberListClose} />
            <FlatList
              data={reactions?.like || []}
