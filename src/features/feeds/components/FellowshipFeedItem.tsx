...
import {
  getCompactFellowshipContents,
} from '@/features/fellowship/utils';
...
export function FellowshipFeedItem({ item }: { item: FellowshipFeed }) {
  ...
  const compactFellowshipContents = useMemo(() => getCompactFellowshipContents(
    item.data.content,
    findMemberInfo,
...
function ContentItem({ item }: { item: ReturnType<typeof getCompactFellowshipContents>[number] }) {
    ...
    <VStack space="xs">
      ...
      <HStack className="gap-[2px] items-center">
        <HStack space="xs" className="items-center">
          <Avatar size="2xs" photoUrl={item.content.member?.photoUrl || undefined} />
