...
export function FeedHomeHeader() {
  ...
  const handlePressGroupItem = useCallback((groupId: string) => {
    ...
    <>
      ...
      <BottomSheetContainer>
        <VStack space="md" className="pt-4 pb-12">
          <Heading size="2xl" className="px-5">그룹 선택</Heading>
          {groups?.map((group) => (
            <AnimatedPressable
              key={group.id}
            ...
            >
              ...
              >
                <HStack space="md" className="items-center">
                  ...
                  />
                  <Text size="lg" weight="medium" className="text-typography-700">{group.groupName}</Text>
                </HStack>
                <Icon as={ChevronRight} size="lg" className="text-typography-600" />
              </HStack>
            </AnimatedPressable>
          ))}
        </VStack>
