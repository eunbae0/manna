    ...
    <View className="relative w-full h-[200px]">
      ...
      /> : (
        <View className="h-full justify-center items-center bg-gray-100 rounded-lg">
          ...
          {isLeader && <Text size="sm" className="text-typography-500 mt-2">
      ...
      {/* 페이지 인디케이터 */}
      {hasCoverImages && <View className="absolute bottom-3 w-full flex-row justify-center items-center z-20">
      ...
      {isLeader && <AnimatedPressable scale="xs" onPress={handlePressManageImages}>
        <HStack space="xs" className="absolute bottom-2 right-3 items-center z-30 rounded-md px-2.5 py-1 bg-gray-900/40">
          <ButtonIcon size="md" as={ImageIcon} className="text-white" />
          <Text size="sm" className="text-white">이미지 관리</Text>
        </HStack>
      </AnimatedPressable>}
    </View>
  )
