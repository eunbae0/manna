    ...
    <VStack space="lg" className="w-full">
      ...
      <HStack space="sm">
        <AnimatedPressable withBackground onPress={() => router.push('/(app)/(group)/create-group')}>
          <HStack space="sm" className="px-4 py-2 items-center border border-primary-500 rounded-xl">
            <Icon as={HousePlus} className="text-primary-500" />
            <Text size="md" weight="medium" className="text-primary-500">소그룹 만들기</Text>
          </HStack>
        </AnimatedPressable>
        <AnimatedPressable withBackground onPress={() => router.push('/(app)/(group)/join-group')}>
          <HStack space="sm" className="px-4 py-2 items-center border border-primary-500 rounded-xl">
            <Icon as={UserPlus} className="text-primary-500" />
            <Text size="md" weight="medium" className="text-typography-700">소그룹 참여하기</Text>
          </HStack>
        </AnimatedPressable>
      </HStack>
    </VStack>
