...
export function useFeedback() {
  ...
  // 개발자 계정 확인 쿼리
  const { data: isDeveloperAccount = false, error: developerStatusError } = useQuery({
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeChatRoomMessages(chatRoomId, (updatedMessages) => {
      queryClient.setQueryData([...CHAT_ROOM_MESSAGES_KEY, chatRoomId], updatedMessages);
    });
  ...
  // 오류 집계
  const error = developerStatusError || messagesError || chatRoomsError || sendError;
