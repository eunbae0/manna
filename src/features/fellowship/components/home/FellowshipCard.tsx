...
export const AnimatedContentItem = memo(function AnimatedContentItem({
  ...
  return (
    <AnimatedVStack space="xs" style={animatedStyle}>
      <Text size="lg" numberOfLines={2} weight="medium" className="text-typography-600">{item.answer}</Text>
      <HStack space="xs" className="items-center">
        <Avatar size="2xs" photoUrl={item.member.photoUrl} />
        <Text size="md" weight="regular" className="text-typography-600">{item.member.displayName}</Text>
      </HStack>
    </AnimatedVStack>
  );
});
...
export default function FellowshipCard({
  ...
  const findMemberInfo = useCallback((id: string) => {
    return fellowship.info.participants.find((participant) => participant.id === id)
  }, [fellowship])
  ...
  }> = useMemo(() => Object.values(fellowship.content.categories)
    .filter((category) => category.isActive && Object.values(Object.values(category.items)[0]?.answers || {}).length > 0)
    .map((category) => ({
      id: category.id,
  ...
  const [currentId, setCurrentId] = useState<string | null>(fellowshipContentTitleList[0]?.id || null)

  const fellowshipContentItemList: Array<FellowshipContentItem> = useMemo(() =>
    fellowshipContentTitleList.map((content) => ({
      id: content.id,
      content: Object.entries(
        Object.values(content.items).sort((a, b) => a.order - b.order)[0]?.answers || {}
      ).map(([memberId, answer]) => ({
  ...
  const currentFellowshipContentItem: FellowshipContentItem['content'] = useMemo(() =>
    currentId
      ? fellowshipContentItemList.find(item => item.id === currentId)?.content || []
      : []
  ...
  return (
    <AnimatedPressable scale="sm" onPress={handlePressCard}>
      <VStack space="md" className="bg-gray-50 border border-gray-100 w-96 min-h-72 rounded-2xl p-4">
        <HStack space="xs" className="items-center justify-between">
          <Heading size="xl" className="max-w-48" numberOfLines={1}>{fellowship.info.title}</Heading>
          <Text className="text-typography-500">{formatLocalDate(fellowship.info.date)} 나눔</Text>
        </HStack>
        {fellowshipContentTitleList.length > 0 ? (
          <VStack space="sm">
            ...
            <VStack space="md">
              {currentFellowshipContentItem?.slice(...generateRandomSliceRange(currentFellowshipContentItem.length))?.map((item, index) => (
                <AnimatedContentItem
                  key={item.member.id}
                  item={item}
                  index={index}
          ...
          <VStack space="sm" className="pt-6">
            <Text size="lg" weight="semi-bold" className="text-typography-700">나눔 답변이 없어요.</Text>
            <Text size="md" weight="medium" className="text-typography-500">지금 클릭해서 그룹원들의 답변을 적어보세요</Text>
          </VStack>
        )}
      </VStack>
    </AnimatedPressable>
