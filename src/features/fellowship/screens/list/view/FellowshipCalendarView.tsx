...
import { useCalendarFellowshipDates, useSelectedDateFellowships } from '@/features/fellowship/hooks/useCalendarFellowships';
import Heading from '@/shared/components/heading';
import FellowshipListItem from '@/features/fellowship/components/FellowshipListItem';
import { useDelayedValue } from '@/hooks/useDelayedValue';
import { FellowshipListSkeleton, FellowshipListSkeletonGroup } from '@/features/fellowship/components/FellowshipListSkeleton';
...
export default function FellowshipCalendarView({ handlePressFellowship }: Props) {
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [currentMonth, setCurrentMonth] = useState<Date>(new Date());
  const [showLeader, setShowLeader] = useState(false);
  const [refreshing, setRefreshing] = useState(false);

  const { data: fellowshipDates, refetch: refetchFellowshipDates } = useCalendarFellowshipDates(
  ...
  const getFellowshipDatesInMonth = useCallback((year: number, month: number) => {
      ...
      ListEmptyComponent={
        showSkeleton ? (
          ...
          <VStack className="items-center justify-center py-8" >
            <Text className="text-typography-500">선택한 날짜에 나눔이 없어요.</Text>
          </VStack>
        ) : null}
