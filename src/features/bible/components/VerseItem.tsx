...
export const VerseItem = memo(
	({
		...
		const isSelected = useMemo(
			() => selectedVerses.includes(item.verse),
			[selectedVerses, item.verse],
		);
		...
		const highlight = useMemo(() => {
			return currentHighlights.find(
				(highlight) => highlight.identifier.verse === item.verse,
			);
		...
		return (
			<AnimatedPressable
				...
				pressableClassName={cn(
					'px-4 py-3',
					isSelected ? 'bg-background-200/80' : '',
				)}
					...
					<Text
						...
						style={{
							...
							textDecorationColor: highlight
								? HIGHLIGHT_COLOR_HEX_MAP[highlight.color || 'yellow']
								: 'none',
						}}
					>
						{item.text}
