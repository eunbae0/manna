...
export function BibleSelector({
	...
	const bookList = useMemo(
		() => getBookList(bookIndex, listType),
		[bookIndex, listType],
	);
	...
	const handlePressBookListItem = useCallback(
		(bookId: string) => {
	...
	const handlePressChapterListItem = useCallback(
	...
	const handlePressVerseListItem = useCallback(
		(verse: number) => {
	...
	const handlePressAddVerses = useCallback(() => {
		...
		const bookName =
			bookIndex.find((book) => book.id === currentBookId)?.name_kr || '';
			...
			<VStack space="md" className="flex-1 justify-between pt-1 pb-2">
				<VStack space="xl" className="flex-1">
					<VStack space="sm">
						...
						{step === 'book' && (
							<HStack space="xs" className="pl-5 pr-3">
								<SegmentedControl
									defaultValue={currentBookType}
									onValueChange={(value) =>
										setCurrentType(value as 'OT' | 'NT')
									}
									className="flex-1"
								>
							...
							<HStack className="items-center pl-2 pr-4 justify-between">
								<HStack className="items-center">
									...
									<Text
										...
										className="text-typography-800"
									>
										{selectedBookName} {currentChapter}장{' '}
...
function getBookList(bookIndex: BookIndex, listType: 'Group' | 'Alphabet') {
	...
	const filterByAlphabet = (book: BookIndexData[], alphabet: string) => {
		return book
			.filter((book) =>
				getChoseong(book.name_kr).startsWith(getChoseong(alphabet)),
			)
