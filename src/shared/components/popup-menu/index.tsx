...
const PopupMenu: React.FC<PopupMenuProps> = ({
		...
		if (triggerRef.current) {
			triggerRef.current.measureInWindow((x, y, width, height) => {
				...
				const estimatedMenuWidth =
					menuLayout.width > 0 ? menuLayout.width : 200;
				const estimatedMenuHeight =
					menuLayout.height > 0 ? menuLayout.height : 150;
		...
		return React.Children.map(children, (child) => {
			if (React.isValidElement(child)) {
				return React.cloneElement(
					child as React.ReactElement<PopupMenuItemProps>,
					{
						onPress: () => {
	...
	return (
		<View>
			<View ref={triggerRef}>{trigger({ onPress: handleToggle })}</View>
