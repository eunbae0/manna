...
export function mapFirebaseError(error: any): ApiError {
  const firebaseCode = error?.code;
  const apiErrorCode = firebaseCode && FIREBASE_ERROR_MAP[firebaseCode] || ErrorCode.AUTH_UNKNOWN;

  return new ApiError({
    code: apiErrorCode,
    message: error?.message || '알 수 없는 인증 오류가 발생했습니다',
    originalError: error,
